...
## Issue with Sandbox
Give the playbooks a try in the Sandbox. If someone has modified or deleted the Sandbox devices it may cause failures for these playbooks; If you observe these errors, you can restore the connected devices from a specific backup "ansiblebackup", see below. See the system settings. Select the ansiblebackup or if not available anymore, select the baseline backup for restore. This backup step takes about 5 minutes to complete.
